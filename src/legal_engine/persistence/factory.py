"""Settings-driven factory for the StatuteRepository/UserRepository,
mirroring knowledge_graph/factory.py's pattern for the graph/vector/
embedder backends.
...
the import itself is deferred to inside each function's "sql" branch.
...
from legal_engine.persistence.user_repository import InMemoryUserRepository, UserRepository
...
def build_user_repository() -> UserRepository:
    if settings.user_backend == "sql":
        try:
            from legal_engine.persistence.sql_repository import SqlAlchemyUserRepository

            return SqlAlchemyUserRepository(settings.postgres_dsn)
        except Exception as exc:
            raise ImportError(
                "The 'sql' user backend requires: pip install 'legal-engine[postgres]' "
                f"(underlying error: {exc.__class__.__name__}: {exc})"
            ) from exc
